...
Bomb::Bomb(float x, float y,float z,double ss, double rot, double rr,int tt, color_t color) {
    ...
    this->object2 = create3DObject(GL_TRIANGLES, 3*n, g_vertex_buffer_data, color, GL_FILL);
}
...
void Bomb::draw(glm::mat4 VP) {
    ...
    glm::mat4 rotate    = glm::rotate((float) (this->rot * M_PI / 180.0f), glm::vec3(1, 0, 0));
...
void Bomb::tick() {
    this->position.x += this->zspeed*sin(this->rotationpp);
    this->position.y += -this->zspeed*cos(this->rotationpp)*sin(this->rotation);
    this->position.z += this->zspeed*cos(this->rotationpp)*cos(this->rotation);
   // printf("%f\n",this->zspeed);
   if(this->type)
